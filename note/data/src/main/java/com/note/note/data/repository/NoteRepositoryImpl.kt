...
import com.note.core.database.extension.safeDbCallAsFlow
...
internal class NoteRepositoryImpl @Inject constructor(
    ...
    override fun getNoteFlow(noteId: Long): Flow<Result<Note?, DataError.Local>> {
        return noteReadDao.getNoteFlow(noteId).safeDbCallAsFlow {
            it?.toNote()
        }.flowOn(ioDispatcher)
    }

    override suspend fun getNote(noteId: Long): Result<Note?, DataError.Local> {
        return withContext(ioDispatcher) {
            safeDbCall { noteReadDao.getNote(noteId)?.toNote() }
        }
    }

    override suspend fun getLastContentId(): Result<Long, DataError.Local> {
        return withContext(ioDispatcher) {
            safeDbCall { noteContentDao.getLastContentId() ?: 1 }
        }
    }
    ...
    override suspend fun updateBranchState(
        ...
        noteState: NoteState
         ...
         withContext(ioDispatcher) {
            noteContentDao.updateBranchState(branchState.code, contentId, noteState.code)
        }
    }
}